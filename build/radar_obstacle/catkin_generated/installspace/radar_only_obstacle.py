...
class RadarOnlyObstacle:
    ...
    def __init__(self):
        ...
        self.target_distance = float(
            rospy.get_param("~target_distance", 1.3))
        self.distance_tolerance = float(
            rospy.get_param("~distance_tolerance", 0.3))

        # ===== 可配置参数：角度 =====
        self.max_angle_deg = float(
            rospy.get_param("~max_angle_deg", 60.0))

        # ===== 可配置参数：RCS =====
        self.rcs_min = float(
            rospy.get_param("~rcs_min", -10.0))

        # ===== 可配置参数：聚类 =====
        self.cluster_radius = float(
            rospy.get_param("~cluster_radius", 0.3))

        # ===== 可配置参数：持久性 =====
        self.persistence_frames = int(
            rospy.get_param("~persistence_frames", 3))
        self.persistence_window = int(
            rospy.get_param("~persistence_window", 5))

        # ===== 可配置参数：清除 =====
        self.clear_timeout = float(
            rospy.get_param("~clear_timeout", 0.5))
        self.clear_cooldown = float(
            rospy.get_param("~clear_cooldown", 1.0))
        ...
        self._check_timer = rospy.Timer(
            rospy.Duration(0.1), self._clear_check_callback)
    ...
    def _publish_obstacles(self, points: list, radar_header: Header):
        ...
        marker_array.markers.append(clear_m)
